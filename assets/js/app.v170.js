(() => {
  ...
  function renderFixtures() {
    ...
    container.innerHTML = visibleFixtures.map((fixture) => {
      ...
      const activePick = enginePickForFixture(fixture);
      const qualified = Boolean(activePick?.qualified);
      const confidence = activePick ? percent(analysis.confidence, 1) : "Processing";
      ...
      return `<button class="fixture-item ${fixture.id === selectedId ? "active" : ""}" data-id="${escapeHtml(fixture.id)}">
        ...
        <div class="fixture-decision">
          ...
          <strong>${escapeHtml(activePick ? analysis.primary.label : "Preparing prediction")}</strong>
        </div>
        <div class="fixture-bottom">
          <span>${escapeHtml(activePick?.market || "PapaSense is calculating")}</span>
          <b>${confidence}</b>
        </div>
