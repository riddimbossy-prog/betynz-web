(() => {
  ...
  function normalizeDashboard(payload) {
    ...
    const issueMap = new Map(
      (payload.generation?.skipped || []).flatMap((issue) => [
        [String(issue.fixtureId), issue],
        [String(issue.externalFixtureId), issue]
      ])
    );
    const models = detailedFixtures.map((fixture) => {
      const model = modelFromFixture(
        fixture,
        byFixture.get(String(fixture.fixtureId))
      );
      model.generationIssue =
        issueMap.get(String(fixture.id)) ||
        issueMap.get(String(fixture.fixtureId)) ||
        null;
      return model;
    });
  ...
  const ENGINE_META = {
    primary: {
      name: "Papa Primary",
      label: "★ PAPA PRIMARY",
      marketLabel: "PAPA PRIMARY MARKET",
      explanation: "Why Papa Primary Chose This Pick"
    },
    aggressive: {
      name: "Aggressive",
      label: "⚡ AGGRESSIVE",
  ...
  function renderFixtures() {
    ...
    container.innerHTML = visibleFixtures.map((fixture) => {
      ...
      const confidence = activePick
        ? percent(analysis.confidence, 1)
        : issue
          ? "No pick"
          : "Hydrating";
      const mode = !activePick
        ? issue?.code === "INSUFFICIENT_INDIVIDUAL_HISTORY"
          ? "History unavailable"
          : "Hydrating history"
        : `${engineMeta().name} · ${qualified ? "Qualified" : "Directional"}`;
      const modeClass = !activePick ? "pending" : qualified ? "qualified" : "directional";
      ...
      return `<button class="fixture-item ${fixture.id === selectedId ? "active" : ""}" data-id="${escapeHtml(fixture.id)}">
        ...
        <div class="fixture-decision">
          ...
          <strong>${escapeHtml(
            activePick
              ? analysis.primary.label
              : issue
                ? "No synthetic prediction published"
                : "Building individual team profiles"
          )}</strong>
        </div>
        <div class="fixture-bottom">
          <span>${escapeHtml(
            activePick?.market ||
            issue?.message ||
            "Fetching completed team history"
          )}</span>
          <b>${confidence}</b>
        </div>
        ${profileAuditLabel(fixture)
          ? `<small class="fixture-audit">${escapeHtml(profileAuditLabel(fixture))}</small>`
          : ""}
        <small class="fixture-why">
          ${activePick ? "Click for full HT/FT reasoning →" : "Waiting for individual history"}
        </small>
      </button>`;
    }).join("");
  ...
  async function loadDashboard({ silent = false } = {}) {
    ...
    try {
      ...
      const hydration = generation.hydration || {};
      const generationNote = generation.attempted
        ? ` · ${hydration.readyTeams || 0}/${hydration.teamsChecked || 0} team profiles ready · ${payload.predictions?.length || 0} real directions`
        : generation.cooldown && generation.missingBefore
          ? ` · ${generation.missingBefore} fixtures still awaiting individual history`
          : "";
      ...
      content.innerHTML = `
        <div class="reason-popup-heading">
          <span class="fixture-mode pending">
            ${issue ? "History unavailable" : "Hydrating history"}
          </span>
          <h2 id="pickReasonTitle">${escapeHtml(fixture.home.name)} vs ${escapeHtml(fixture.away.name)}</h2>
          <p>${escapeHtml(
            issue?.message ||
            "PapaSense is fetching each team's completed matches and building individual HT/FT profiles."
          )}</p>
          <div class="anti-zombie-note">
            No league-default or copied 57% prediction is published without real history from both teams.
          </div>
        </div>`;
      dialog.showModal();
      return;
