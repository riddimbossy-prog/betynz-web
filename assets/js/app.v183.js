      ...
      <div class="background-picks-banner">
        <div>
          ...
          <div>
            ...
            <small>Completed picks appear automatically. Incomplete fixtures are not shown as fake cards.${withheld ? ` ${withheld} fixture${withheld === 1 ? "" : "s"} were withheld for insufficient history.` : ""}</small>
          </div>
        </div>
    ...
    if (!fixtures.length) {
      ...
      container.innerHTML = importedCount
        ? `<div class="picks-preparing-card">
            ...
            <strong>Papa is preparing today’s picks</strong>
            <p>${pendingCount || importedCount} fixture${(pendingCount || importedCount) === 1 ? "" : "s"} are being analysed in the background. Only completed picks will appear here.</p>
  ...
  async function loadDashboard({ silent = false } = {}) {
    ...
    try {
      ...
      setLiveState(
        ...
        fixtures.length ? "Papa’s completed picks are live" : pending ? "Papa is preparing today’s picks" : "Live database connected",
