(function(){
  ...
  function onboardingStep(step){
    const pages=[
      ["⚡","Welcome to Betynz","Eighteen specialist engines analyse supported fixtures. Zeus applies the final consensus and conflict checks."],
      ["A1","Understand the grades","A1 is the strictest board grade. A2 is a strong qualified pick. A specialist signal can still be rejected by Zeus."],
      ["+","Make it yours","Tap + to save a pick, follow engines and favourite leagues or teams. Your official board is never changed by personal actions."],
      ["18+","Use records responsibly","Selections are informational and never guaranteed. Review the complete settled record and take regular breaks."]
    ];
    const [icon,title,copy]=pages[step], modal=$("#onboarding-modal"),content=$("#onboarding-content");if(!modal||!content)return;
    content.innerHTML=`<div class="onboarding-progress">${pages.map((_,i)=>`<i class="${i===step?"active":""}"></i>`).join("")}</div><span class="onboarding-icon">${icon}</span><small>BETYNZ QUICK START</small><h2 id="onboarding-title">${title}</h2><p>${copy}</p><div class="onboarding-actions">${step?'<button type="button" class="secondary-btn" data-onboarding-back>Back</button>':'<button type="button" class="secondary-btn" data-onboarding-skip>Skip</button>'}<button type="button" class="primary-btn" data-onboarding-next="${step}">${step===pages.length-1?"Enter Betynz":"Continue"}</button></div>`;
  }
  function openOnboarding(force=false){if(!force&&localStorage.getItem("betynz-onboarding-v53"))return;$("#onboarding-backdrop").hidden=false;$("#onboarding-modal").hidden=false;document.body.classList.add("onboarding-open");onboardingStep(0)}
  function closeOnboarding(){localStorage.setItem("betynz-onboarding-v53","done");$("#onboarding-backdrop").hidden=true;$("#onboarding-modal").hidden=true;document.body.classList.remove("onboarding-open")}
  ...
  async function renderHealth(){
    ...
    const stale=age==null||age>12, oddsReady=matches.filter(m=>m.odds||m.marketOdds||m.oddsBooks).length;
    const checks=[
  ...
  function renderAccountTools(){const root=$("#account-launch-tools");if(!root)return;root.innerHTML=`<div class="panel-title"><div><h3>Launch & community controls</h3><p>Manage onboarding, local data and your secure account.</p></div></div><div class="account-tool-grid"><button type="button" data-replay-onboarding><span>?</span><b>Replay Quick Start</b><small>See the four launch screens again.</small></button><button type="button" data-export-account><span>⇩</span><b>Export Local Data</b><small>Download preferences, follows and comments.</small></button><button type="button" data-clear-local><span>⌫</span><b>Clear Guest Data</b><small>Remove this device's local Betynz settings.</small></button>${account().signedIn&&backendReady()?'<button type="button" class="danger" data-delete-account><span>×</span><b>Delete Account</b><small>Permanently remove the secure account.</small></button>':""}</div>`}
