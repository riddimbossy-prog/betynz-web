/* ============================================================
   BETYNZ — DATA FETCHER  (API-Football / api-sports.io)
...
(async () => {
  console.log("\n=== Betynz data fetcher (API-Football) ===\n");
  let cfg;
  try { cfg = readConfig(); }
  ...
  for (const grp of leagueGroups) {
    ...
    for (const fx of fixtures) {
      ...
      out.push({
        ...
        homePlayed: H&&H.all&&Number(H.all.played) ? Number(H.all.played) : null,
        awayPlayed: A&&A.all&&Number(A.all.played) ? Number(A.all.played) : null,
        homePts: H.points ?? 0, awayPts: A.points ?? 0,
        homePPG: H&&H.all&&Number(H.all.played) ? round2(Number(H.points||0)/Number(H.all.played)) : null,
        awayPPG: A&&A.all&&Number(A.all.played) ? round2(Number(A.points||0)/Number(A.all.played)) : null,
        homeVenuePPG: H._homeGames ? round2(Number(H._homePts||0)/Number(H._homeGames)) : null,
        awayVenuePPG: A._awayGames ? round2(Number(A._awayPts||0)/Number(A._awayGames)) : null,
  ...
  const previousClean = removePackagedDemoFixtures(existing);
  if (previousClean.removed) console.log(`Purged ${previousClean.removed} packaged demo fixture(s) from the saved board.`);
  existing = previousClean.matches;
  const windowSet = new Set(DATE_WINDOW);
  const kept = existing.filter(x => x.matchDate && !windowSet.has(x.matchDate));
  const mergedClean = removePackagedDemoFixtures(kept.concat(out));
  const merged = mergedClean.matches;
  if (mergedClean.removed) console.log(`Blocked ${mergedClean.removed} packaged demo fixture(s) during merge.`);
