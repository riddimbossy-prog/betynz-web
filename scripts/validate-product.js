...
const required=["index.html","styles.css","app.js","rebel-engine-core.js","olympian-engine-core.js","data.js","manifest.webmanifest","service-worker.js","backend-config.js","backend-client.js","monetization-config.js","CNAME"];
for(const f of required){if(!fs.existsSync(path.join(ROOT,f)))throw new Error(`Missing ${f}`)}
for(const f of ["app.js","backend-client.js","rebel-engine-core.js","olympian-engine-core.js","scripts/build-snapshot.js","scripts/fetch-data.js","scripts/fetch-scores.js","scripts/enrich-rebel-odds.js"]){new vm.Script(fs.readFileSync(path.join(ROOT,f),"utf8"),{filename:f})}
