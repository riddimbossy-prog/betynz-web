...
assert(index.includes("boot.js?v=5.9.3"),"Index must request the repaired boot loader with a fresh cache key.");
assert(boot.includes("community-features.js?v=${VERSION}"),"Boot loader must request the repaired community script.");
assert(boot.includes("app.js?v=${VERSION}"),"Boot loader must request the repaired app script.");
assert(sw.includes("betynz-v5.9.3"),"Service worker cache must be bumped.");
assert(app.includes("if(allPicksMemo)return allPicksMemo"),"The full pick list must be memoized for populated boards.");
assert(app.includes("betynz-sw-reload-v593"),"PWA activation must have a one-reload guard.");
